...
import { type RegisterFormValues, useRegisterForm, RegisterFormNames } from '../../model/schema/useRegisterForm'
      ...
      <VStack className={cls.auth} gap={12}>
        <VStack className={cls.authTop} align='center'>
          ...
          <VStack align='center' gap={16} className={cls.formTop}>
            <Text as='span' size={14}>{t('Зарегистрируйтесь, чтобы смотреть фото и видео ваших друзей.')}</Text>
            <Button
              variant='outline'
              max
          ...
          >
            <VStack gap={12} align='center'>
              ...
              <Text size={14} color='gray' fw={400} className={cls.desc}>
                {t('Регистрируясь, вы принимаете Условия. Прочитайте Политика конфидициальности, чтобы узнать, как мы получаем, используем и передаем ваши данные. Также просмотрите Политику в отношении файлов cookie, чтобы узнать, как мы используем файлы cookie и подобные технологии.')}
              </Text>
              {authError && <Text className={cls.formError} color='red'>{authError}</Text>}
              <Button disabled={!isValid && authLoading} type='submit' max loading={authLoading}>{t('Войти')}</Button>
              <VStack gap={16} align='center'>
                <AppLink to='/forgot'><Text>{t('Забыли пароль?')}</Text></AppLink>
              </VStack>
            </VStack>
          </Form>
        </VStack>
        ...
        <HStack className={cls.authBottom} align='center' gap={12} justify='center'>
          <Text as='span' size={13}>{t('Есть аккаунт?')}</Text>
          <AppLink to='/login'>
            <Text color='blue' size={14}>{t('Вход')}</Text>
          </AppLink>
        </HStack>
      </VStack>
    </HStack>
